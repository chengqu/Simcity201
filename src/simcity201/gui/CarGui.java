...
//    }
    ...
    public void DoDriveTo(String startDest,String dest){
    	hide  = false;
    	astar.astar(astar.map.get(startDest), astar.map.get(dest));
    	while(!astar.path.isEmpty()){
    		if(xPos != astar.path.get(0).x || yPos != astar.path.get(0).y){
    			xDestination = astar.path.get(0).x;
    			yDestination = astar.path.get(0).y;
    		}
    		else if(xPos == astar.path.get(0).x && yPos == astar.path.get(0).y)
    			astar.path.remove(0);
    	}
