...
import LYN.WaiterProducer.MyCustomer;
import LYN.WaiterProducer.State;
...
import LYN.test.mock.MockWaiter;
...
public class WaiterProducerTest extends TestCase
{
	...
	public void setUp() throws Exception{
		...
		waitergui = new WaiterGui(waiter);
		pm = new ProducerConsumerMonitor<CookAgent.Order>(30);
		p = new Person("josh",true);
		cashier = new CashierAgent("cashier",rp); 
		cook = new CookAgent("cook",cashier,rp,pm);
		waiter = new WaiterProducer(p,"waiter",pm,rp);
		customer = new CustomerAgent(p,"hello");                
		customer.setGui(gui1);
	...
	public void testSendOrdertoCook() {
		...
		assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
				,waiter.customers.get(0).s,State.ordered);
		...
		assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
				,waiter.customers.get(0).s, State.orderisready);
		assertTrue("Cashier's scheduler should have returned true (needs to react to customer's ReadyToPay), but didn't.", 
				waiter.pickAndExecuteAnAction());
